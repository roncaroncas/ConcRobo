...
class ConectadoView(tk.Frame):
	...
	def __init__(self, parent, controller, gui):
		...
		self.varIm_pres 	= ImageTk.PhotoImage(self.images['pressure']['bar-1'])
	...
	def draw(self):
		...
		######ID
		tk.Label(
			self, self.gui.border, textvariable=self.var_id, font=self.gui.h2_font, width=1, height=2
			).grid(
		...
		pingFrame.columnconfigure(0, weight=1)
		...
		xyzFrame.columnconfigure(0,minsize=50)
		xyzFrame.columnconfigure(1,weight=1)
		...
		measureFrame.columnconfigure(2,weight=1)
		...
		warnFrame.columnconfigure(1, weight=1)
		...
		self.varEn_warn = tk.Entry(warnFrame, width=10, font=self.gui.p_font, borderwidth=2, relief="groove")
		...
		tk.Label(
			warnFrame, width=10, font=self.gui.p_font,textvariable=self.var_lastWarn, anchor='w', borderwidth=2, relief="groove"
			).grid(
			self.gui.p_pad_compact, row=row+1, column=1, sticky="WENS")
		...
		_, row = self.grid_size()
		tk.Button(
			self, text="Desconectar", command=self.controller.b_Voltar,
